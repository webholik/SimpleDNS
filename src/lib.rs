...
impl BytePacketBuffer {
    ...
    pub fn read_qname(&mut self, out_str: &mut String) -> Result<(), Error> {
        ...
        loop {
            ...
            if len & 0xC0 == 0xC0 {
                ...
                let buf = self.get_range(pos, len as usize)?;
                out_str.push_str(&String::from_utf8_lossy(buf).to_lowercase());
                delim = ".";
                pos += len as usize;
            }
        }
    ...
    pub fn set(&mut self, pos: usize, val: u8) -> Result<(), Error> {
        if pos >= 512 {
            return Err(Error::new(ErrorKind::InvalidInput, "pos out of range"));
        }
...
impl DnsRecord {
    ...
    pub fn write(&self, buffer: &mut BytePacketBuffer) -> Result<usize, Error> {
        ...
        match *self {
            ...
            DnsRecord::UNKNOWN { .. } => {
                println!("Skipping this record: {:?}", self);
            }
        }
